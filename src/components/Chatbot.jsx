...
const Chatbot = ({ isOpen, onClose }) => {
  ...
  const faqResponses = {
    "how to register":
      'To register for an event, visit the event page and click the "Register" button. You\'ll need to be logged in to your Nondan account.',
    "create account":
      'You can create an account by clicking "Sign Up" in the top navigation. Choose between Student or Admin account types.',
    "event registration":
      "Event registration is free for most events. Some premium events may require payment. You'll see the cost (if any) on the event page.",
    certificates:
      'You can download your certificates from the "Certificates" page in your student dashboard after completing events.',
    "hub membership":
      'To join a hub, visit the hub page and click "Join Hub". Some hubs may require approval from administrators.',
    "forgot password":
      'Click "Forgot Password" on the login page and enter your email. We\'ll send you a reset link.',
    "contact support":
      "You can contact our support team at support@nondan.com or use the contact form in the Settings page.",
    "cancel registration":
      "You can cancel your event registration from your student dashboard up to 24 hours before the event starts.",
    "event creation":
      'Hub administrators can create events through the admin dashboard. Visit "Create Event" in the admin panel.',
    "platform features":
      "Nondan offers event management, hub communities, certificate generation, QR check-ins, and analytics for organizers.",
  };
  ...
  const fetchInitialMessage = async () => {
    ...
    try {
      const response = await fetch(
        "https://nondan-backend.vercel.app/api/ai/aicall",
        {
          method: "POST",
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50 dark:bg-gray-900/50">
          {messages.map((message) => (
            ...
            >
              <div
                className={`flex space-x-2 max-w-[75%] ${
                  message.sender === "user"
                    ? "flex-row-reverse space-x-reverse"
                    : ""
                }`}
